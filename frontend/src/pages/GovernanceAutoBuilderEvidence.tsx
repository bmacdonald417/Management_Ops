...
export default function GovernanceAutoBuilderEvidence() {
  const [markdown, setMarkdown] = useState<string>('');
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    client.get('/governance/auto-builder/evidence').then((r) => {
  ...
  if (loading) return <div className="text-slate-500">Loading evidence packet...</div>;

  return (
    <div>
      <div className="flex justify-between items-center mb-6 print:hidden">
        <h1 className="font-display font-bold text-2xl text-gov-navy">Evidence Packet Preview</h1>
