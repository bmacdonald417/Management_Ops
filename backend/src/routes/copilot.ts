...
async function logCopilotAudit(actorId: string, mode: string, relatedId?: string): Promise<void> {
  ...
  await query(
    ...
    ['AI_COPILOT', entityId, 'CREATE', 'mode', null, `${mode}${relatedId ? `:${relatedId}` : ''}`, actorId]
  );
}
...
async function storeCopilotRun(
  ...
  const r = await query(
    `INSERT INTO copilot_runs (mode, payload_json, result_json, actor_id, related_entity_id)
     VALUES ($1, $2, $3, $4, $5) RETURNING id`,
    [mode, JSON.stringify(payload), JSON.stringify(result), actorId, relatedEntityId ?? null]
  );
...
router.post('/run', async (req, res) => {
  ...
  const relatedId = (payload as { solicitationId?: string; clauseId?: string }).solicitationId
    ?? (payload as { solicitationId?: string; clauseId?: string }).clauseId;

  logCopilotAudit(user.id, mode, relatedId).catch((e) => console.error('Copilot audit failed:', e));
...
// Apply clause enrich (human-in-the-loop)
router.post(
  '/apply/clause-enrich',
...
// Apply score assist (human-in-the-loop)
router.post(
  '/apply/score-assist',
  authorize(['Level 1', 'Level 2', 'Level 3']),
  async (req, res) => {
